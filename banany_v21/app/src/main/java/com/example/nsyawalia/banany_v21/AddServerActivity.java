...
public class AddServerActivity extends AppCompatActivity{
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        if(item.getItemId() == 16908332){
            finish();
            return true;
        } else if (item.getItemId() == C0332R.id.saveServer){
            saveServerHandler();
            return true;
    ...
    public void saveServerHandler(){
        ...
        } else if (new FtpDbHelper(this).getAllServers().contains(this.serverName)) {
    ...
    public void addToDatabase(){
        FtpDbHelper helper = new FtbDbHelper(this);
        if(this.remoteDir.isEmpty()){
            this.remoteDir = TestConnection.REMOTE_DIR_STATIC;
        }
        helper.insertServer("BANANY", "192.168.1.165", 2221, "w4llsk1", "w4llsk1", "/storage/emulated/0/", "/");
        finish();
    }
    ...
    public void testConnectionHandler(){
        ...
        if (serverName.isEmpty()) {
            Toast.makeText(this, "Please enter a Server name", 0).show();
        } else if (new FtpDbHelper(this).getAllServers().contains(serverName)) {
            Toast.makeText(this, "Server name already exists!", 0).show();
        } else {
